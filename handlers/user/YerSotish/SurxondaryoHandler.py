...
from keyboards.inline.data import SurxonYerData
...
surxon_yer_router = Router()

@surxon_yer_router.callback_query(SurxonYerData.filter(F.word=="surxonyer"))
async def first(callback_query: CallbackQuery, state: FSMContext, callback_data: SurxonYerData):
    # await callback_query.answer(hovlitanlandi)
    await callback_query.message.answer(rasmlar, parse_mode="HTML")

    await state.set_state(SurxondaryoYerSotish.images)


@surxon_yer_router.message(SurxondaryoYerSotish.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(SurxondaryoYerSotish.umumiyMaydon)


@surxon_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    SurxondaryoYerSotish.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@surxon_yer_router.message(SurxondaryoYerSotish.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(SurxondaryoYerSotish.gaz)


# ================================================================

@surxon_yer_router.callback_query(BorData.filter(F.word=="bor"), SurxondaryoYerSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(SurxondaryoYerSotish.svet)


@surxon_yer_router.callback_query(YoqData.filter(F.word=="yoq"), SurxondaryoYerSotish.gaz)
...
@surxon_yer_router.callback_query(BorData.filter(F.word=="bor"), SurxondaryoYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Свет ✔️"
    # await callback_query.answer("Танланди")
    ...
    await state.set_state(SurxondaryoYerSotish.suv)


@surxon_yer_router.callback_query(YoqData.filter(F.word=="yoq"), SurxondaryoYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Танланди")
...
@surxon_yer_router.callback_query(BorData.filter(F.word=="bor"), SurxondaryoYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Сув ✔️"
    ...
    await state.set_state(SurxondaryoYerSotish.kanal)


@surxon_yer_router.callback_query(YoqData.filter(F.word=="yoq"), SurxondaryoYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@surxon_yer_router.callback_query(BorData.filter(F.word=="bor"), SurxondaryoYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Канализация ✔️"
    # await callback_query.answer("Tanlandi")
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(SurxondaryoYerSotish.qoshimchaMalumot)
    else:
        await state.set_state(SurxondaryoYerSotish.qoshimchaMalumot)


@surxon_yer_router.callback_query(YoqData.filter(F.word=="yoq"), SurxondaryoYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@surxon_yer_router.message(SurxondaryoYerSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(SurxondaryoYerSotish.hujjatlar)


# =================================================================
    
@surxon_yer_router.callback_query(DocumentHaveData.filter(F.word=="dokumentbor"), SurxondaryoYerSotish.hujjatlar)
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SurxondaryoYerSotish.valyuta)


@surxon_yer_router.callback_query(DocumentNotData.filter(F.word=="dokumentyoq"), SurxondaryoYerSotish.hujjatlar)
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Тайёр эмас"
...
@surxon_yer_router.callback_query(USDData.filter(F.word=="usd"), SurxondaryoYerSotish.valyuta)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(SurxondaryoYerSotish.narxi)


@surxon_yer_router.callback_query(SUMData.filter(F.word=="sum"), SurxondaryoYerSotish.valyuta)
...
@surxon_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    SurxondaryoYerSotish.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@surxon_yer_router.message(SurxondaryoYerSotish.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(SurxondaryoYerSotish.manzil)


@surxon_yer_router.message(SurxondaryoYerSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(SurxondaryoYerSotish.moljal)


@surxon_yer_router.message(SurxondaryoYerSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(SurxondaryoYerSotish.telNumberOne)


@surxon_yer_router.message(SurxondaryoYerSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(SurxondaryoYerSotish.telNumberTwo)
    else:
        await state.set_state(SurxondaryoYerSotish.telNumberTwo)


@surxon_yer_router.message(SurxondaryoYerSotish.telNumberTwo)
async def telNumbertwo(message: types.Message, state: FSMContext):
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [surxondaryoregion, data2, data3, data9, gaz, svet, suv, kanal, data10, document,
        ...
        await state.set_state(SurxondaryoYerSotish.check)
...
@surxon_yer_router.message(SurxondaryoYerSotish.check)
...
@surxon_yer_router.callback_query(IsGroupCall(), TasdiqlashSurxon.filter(F.word == "surxontasdiqlash"))
...
@surxon_yer_router.callback_query(IsGroupCall(), BekorQilishSurxon.filter(F.word=="surxonbekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
