...
from keyboards.inline.HomeButton import jizzax_group
...
from keyboards.inline.data import TasdiqlashJizzax, BekorQilishJizzax
...
async def check(message: types.Message, state: FSMContext, bot: Bot):
    ...
    if mycheck == "✅ Эълонни жойлаш":
        ...
        if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
            ...
            await bot.send_message(chat_id=group_id, text="Буйруқ Беринг", reply_markup=jizzax_group)
...
@jizzax_yer_router.callback_query(IsGroupCall(), TasdiqlashJizzax.filter(F.word == "jizzaxtasdiqlash"))
...
@jizzax_yer_router.callback_query(IsGroupCall(), BekorQilishJizzax.filter(F.word=="jizzaxbekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
