...
from keyboards.inline.data import FargonaYerData
...
@fargona_yer_router.message(FargonaYerSotish.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(FargonaYerSotish.umumiyMaydon)


@fargona_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaYerSotish.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@fargona_yer_router.message(FargonaYerSotish.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(FargonaYerSotish.gaz)
...
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(FargonaYerSotish.svet)
...
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(FargonaYerSotish.suv)
...
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(FargonaYerSotish.kanal)
...
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(FargonaYerSotish.qoshimchaMalumot)
    else:
        await state.set_state(FargonaYerSotish.qoshimchaMalumot)
...
@fargona_yer_router.message(FargonaYerSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(FargonaYerSotish.hujjatlar)
...
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(FargonaYerSotish.valyuta)
...
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(FargonaYerSotish.narxi)
...
@fargona_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaYerSotish.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@fargona_yer_router.message(FargonaYerSotish.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(FargonaYerSotish.manzil)


@fargona_yer_router.message(FargonaYerSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(FargonaYerSotish.moljal)


@fargona_yer_router.message(FargonaYerSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(FargonaYerSotish.telNumberOne)


@fargona_yer_router.message(FargonaYerSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(FargonaYerSotish.telNumberTwo)
    else:
        await state.set_state(FargonaYerSotish.telNumberTwo)


@fargona_yer_router.message(FargonaYerSotish.telNumberTwo)
async def telNumbertwo(message: types.Message, state: FSMContext):
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [fargonaregion, data2, data3, data9, gaz, svet, suv, kanal, data10, document,
        ...
        await state.set_state(FargonaYerSotish.check)
...
@fargona_yer_router.message(FargonaYerSotish.check)
...
@fargona_yer_router.callback_query(IsGroupCall(), TasdiqlashFargona.filter(F.word == "fargonatasdiqlash"))
...
@fargona_yer_router.callback_query(IsGroupCall(), BekorQilishFargona.filter(F.word=="fargonabekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
