...
from keyboards.inline.data import QashqaYerData
...
@qashqa_yer_router.callback_query(QashqaYerData.filter(F.word=="qashqayer"))
async def first(callback_query: CallbackQuery, state: FSMContext, callback_data: QashqaYerData):
    # await callback_query.answer(hovlitanlandi)
    await callback_query.message.answer(rasmlar, parse_mode="HTML")

    await state.set_state(QashqadaryoYerSotish.images)


@qashqa_yer_router.message(QashqadaryoYerSotish.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(QashqadaryoYerSotish.umumiyMaydon)


@qashqa_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoYerSotish.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@qashqa_yer_router.message(QashqadaryoYerSotish.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoYerSotish.gaz)


# ================================================================

@qashqa_yer_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoYerSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(QashqadaryoYerSotish.svet)


@qashqa_yer_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoYerSotish.gaz)
...
@qashqa_yer_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Свет ✔️"
    # await callback_query.answer("Танланди")
    ...
    await state.set_state(QashqadaryoYerSotish.suv)


@qashqa_yer_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoYerSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Танланди")
...
@qashqa_yer_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Сув ✔️"
    ...
    await state.set_state(QashqadaryoYerSotish.kanal)


@qashqa_yer_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoYerSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@qashqa_yer_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Канализация ✔️"
    # await callback_query.answer("Tanlandi")
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(QashqadaryoYerSotish.qoshimchaMalumot)
    else:
        await state.set_state(QashqadaryoYerSotish.qoshimchaMalumot)


@qashqa_yer_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoYerSotish.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@qashqa_yer_router.message(QashqadaryoYerSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoYerSotish.hujjatlar)


# =================================================================
    
@qashqa_yer_router.callback_query(DocumentHaveData.filter(F.word=="dokumentbor"), QashqadaryoYerSotish.hujjatlar)
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(QashqadaryoYerSotish.valyuta)


@qashqa_yer_router.callback_query(DocumentNotData.filter(F.word=="dokumentyoq"), QashqadaryoYerSotish.hujjatlar)
async def dokumentlar(callback_query: types.CallbackQuery, state: FSMContext):
    text = "Тайёр эмас"
...
@qashqa_yer_router.callback_query(USDData.filter(F.word=="usd"), QashqadaryoYerSotish.valyuta)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(QashqadaryoYerSotish.narxi)


@qashqa_yer_router.callback_query(SUMData.filter(F.word=="sum"), QashqadaryoYerSotish.valyuta)
...
@qashqa_yer_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoYerSotish.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@qashqa_yer_router.message(QashqadaryoYerSotish.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoYerSotish.manzil)


@qashqa_yer_router.message(QashqadaryoYerSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(QashqadaryoYerSotish.moljal)


@qashqa_yer_router.message(QashqadaryoYerSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(QashqadaryoYerSotish.telNumberOne)


@qashqa_yer_router.message(QashqadaryoYerSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(QashqadaryoYerSotish.telNumberTwo)
    else:
        await state.set_state(QashqadaryoYerSotish.telNumberTwo)


@qashqa_yer_router.message(QashqadaryoYerSotish.telNumberTwo)
async def telNumbertwo(message: types.Message, state: FSMContext):
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [qashqadaryoregion, data2, data3, data9, gaz, svet, suv, kanal, data10, document,
        ...
        await state.set_state(QashqadaryoYerSotish.check)
...
@qashqa_yer_router.message(QashqadaryoYerSotish.check)
...
@qashqa_yer_router.callback_query(IsGroupCall(), TasdiqlashQashqa.filter(F.word == "qashqatasdiqlash"))
...
@qashqa_yer_router.callback_query(IsGroupCall(), BekorQilishQashqa.filter(F.word=="qashqabekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
