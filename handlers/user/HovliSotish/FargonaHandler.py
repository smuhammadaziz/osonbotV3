...
from keyboards.inline.HomeButton import fargona_group
...
from utils.QuestionHovli.hovliqs import hovlitanlandi, rasmlar, umumiyMaydonyoz, faqatRaqamyoz, gazyoz, \
    ...
    xammomyoz, xonalaryoz, fargona_id, check_text, fargonaregion, data2, data32, data33, \
    data34, data35, success_text, group_id, success_message, fail_message


from keyboards.inline.data import FargonaHovliData
...
from keyboards.inline.data import TasdiqlashFargona, BekorQilishFargona
...
fargona_router = Router()

@fargona_router.callback_query(FargonaHovliData.filter(F.word=="fargonahovli"))
async def first(callback_query: CallbackQuery, state: FSMContext, callback_data: FargonaHovliData):
    # await callback_query.answer(hovlitanlandi)
    await callback_query.message.answer(rasmlar, parse_mode="HTML")

    await state.set_state(FargonaHomeSotishHovli.images)


@fargona_router.message(FargonaHomeSotishHovli.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(FargonaHomeSotishHovli.umumiyMaydon)



@fargona_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaHomeSotishHovli.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@fargona_router.message(FargonaHomeSotishHovli.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(FargonaHomeSotishHovli.xonalar)

    await bot.send_message(chat_id=message.chat.id, text=xonalaryoz, parse_mode="HTML")


@fargona_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaHomeSotishHovli.xonalar)
async def check_xonalar(message: Message):
    await message.reply(faqatRaqamyoz)


@fargona_router.message(FargonaHomeSotishHovli.xonalar)
async def umumiymaydon(message: Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
    ...
    await bot.send_message(chat_id=message.chat.id, text=oshxonayoz, parse_mode="HTML",
                           reply_markup=borYoq)
    await state.set_state(FargonaHomeSotishHovli.oshxona)



# =================================================
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.oshxona)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(FargonaHomeSotishHovli.hammom)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.oshxona)
...
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.hammom)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: BorData):
    text = "бор"
    # await callback_query.answer("Pressed")
    ...
    await state.set_state(FargonaHomeSotishHovli.qavat)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.hammom)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "йўқ"
    # await callback_query.answer("Pressed")
...
@fargona_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaHomeSotishHovli.qavat)
async def check_qavat(message: types.Message):
    await message.reply(faqatRaqamyoz)


@fargona_router.message(FargonaHomeSotishHovli.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(FargonaHomeSotishHovli.remont)


# ====================================================================
@fargona_router.callback_query(YevroremontData.filter(F.word=="yevroremont"), FargonaHomeSotishHovli.remont)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YevroremontData):
    ...
    await state.set_state(FargonaHomeSotishHovli.jihozlar)


@fargona_router.callback_query(TamirlangantData.filter(F.word=="tamirlangan"), FargonaHomeSotishHovli.remont)
...
@fargona_router.callback_query(OrtachaData.filter(F.word=="ortacha"), FargonaHomeSotishHovli.remont)
...
@fargona_router.callback_query(TamirsizData.filter(F.word=="tamirsiz"), FargonaHomeSotishHovli.remont)
...
# ===============================================================
@fargona_router.callback_query(MavjudData.filter(F.word=="mavjud"), FargonaHomeSotishHovli.jihozlar)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: MavjudData):
    ...
    await state.set_state(FargonaHomeSotishHovli.gaz)


@fargona_router.callback_query(JihozlarsizData.filter(F.word=="jihozlarsiz"), FargonaHomeSotishHovli.jihozlar)
...
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(FargonaHomeSotishHovli.svet)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.gaz)
...
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Свет ✔️"
    # await callback_query.answer("Танланди")
    ...
    await state.set_state(FargonaHomeSotishHovli.suv)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Танланди")
...
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Сув ✔️"
    ...
    await state.set_state(FargonaHomeSotishHovli.kanal)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@fargona_router.callback_query(BorData.filter(F.word=="bor"), FargonaHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Канализация ✔️"
    # await callback_query.answer("Tanlandi")
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(FargonaHomeSotishHovli.qoshimchaMalumot)
    else:
        await state.set_state(FargonaHomeSotishHovli.qoshimchaMalumot)


@fargona_router.callback_query(YoqData.filter(F.word=="yoq"), FargonaHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@fargona_router.message(FargonaHomeSotishHovli.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
    ...
    await state.set_state(FargonaHomeSotishHovli.valyuta)


# =================================================================

@fargona_router.callback_query(USDData.filter(F.word=="usd"), FargonaHomeSotishHovli.valyuta)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(FargonaHomeSotishHovli.narxi)


@fargona_router.callback_query(SUMData.filter(F.word=="sum"), FargonaHomeSotishHovli.valyuta)
...
@fargona_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    FargonaHomeSotishHovli.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@fargona_router.message(FargonaHomeSotishHovli.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(FargonaHomeSotishHovli.manzil)


@fargona_router.message(FargonaHomeSotishHovli.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(FargonaHomeSotishHovli.moljal)


@fargona_router.message(FargonaHomeSotishHovli.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(FargonaHomeSotishHovli.telNumberOne)


@fargona_router.message(FargonaHomeSotishHovli.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(FargonaHomeSotishHovli.telNumberTwo)
    else:
        await state.set_state(FargonaHomeSotishHovli.telNumberTwo)


@fargona_router.message(FargonaHomeSotishHovli.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [fargonaregion, data2, data3, data4, oshxona, hammom, data6, data7, data8, data9, gaz, svet, suv, kanal,
                  data10,
        ...
        await state.set_state(FargonaHomeSotishHovli.check)
...
@fargona_router.message(FargonaHomeSotishHovli.check)
async def check(message: types.Message, state: FSMContext):
    ...
    if mycheck == "✅ Эълонни жойлаш":
        ...
        if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
            ...
            await bot.send_message(chat_id=group_id, text="Буйруқ Беринг", reply_markup=fargona_group)
...
@fargona_router.callback_query(IsGroupCall(), TasdiqlashFargona.filter(F.word == "fargonatasdiqlash"))
...
@fargona_router.callback_query(IsGroupCall(), BekorQilishFargona.filter(F.word=="fargonabekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
