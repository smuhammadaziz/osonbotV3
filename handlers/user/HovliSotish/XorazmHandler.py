...
@xorazm_router.callback_query(IsGroupCall(), Tasdiqlash.filter(F.word == "tasdiqlash"))
...
@xorazm_router.callback_query(IsGroupCall(), BekorQilish.filter(F.word=="BekorQilish"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
