...
from keyboards.inline.HomeButton import qashqa_group

from loader import bot, db
from states.HovliState.QashqadaryoState import QashqadaryoHomeSotishHovli
...
from keyboards.inline.data import QashqaHovliData
...
from keyboards.inline.data import TasdiqlashQashqa, BekorQilishQashqa
...
qashqa_router = Router()

@qashqa_router.callback_query(QashqaHovliData.filter(F.word=="qashqadaryohovli"))
async def first(callback_query: CallbackQuery, state: FSMContext, callback_data: QashqaHovliData):
    # await callback_query.answer(hovlitanlandi)
    await callback_query.message.answer(rasmlar, parse_mode="HTML")

    await state.set_state(QashqadaryoHomeSotishHovli.images)


@qashqa_router.message(QashqadaryoHomeSotishHovli.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.umumiyMaydon)



@qashqa_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoHomeSotishHovli.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@qashqa_router.message(QashqadaryoHomeSotishHovli.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.xonalar)

    await bot.send_message(chat_id=message.chat.id, text=xonalaryoz, parse_mode="HTML")


@qashqa_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoHomeSotishHovli.xonalar)
async def check_xonalar(message: Message):
    await message.reply(faqatRaqamyoz)


@qashqa_router.message(QashqadaryoHomeSotishHovli.xonalar)
async def umumiymaydon(message: Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
    ...
    await bot.send_message(chat_id=message.chat.id, text=oshxonayoz, parse_mode="HTML",
                           reply_markup=borYoq)
    await state.set_state(QashqadaryoHomeSotishHovli.oshxona)



# =================================================
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.oshxona)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.hammom)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.oshxona)
...
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.hammom)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: BorData):
    text = "бор"
    # await callback_query.answer("Pressed")
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.qavat)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.hammom)
async def kvartira(callback_query: CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "йўқ"
    # await callback_query.answer("Pressed")
...
@qashqa_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoHomeSotishHovli.qavat)
async def check_qavat(message: types.Message):
    await message.reply(faqatRaqamyoz)


@qashqa_router.message(QashqadaryoHomeSotishHovli.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.remont)


# ====================================================================
@qashqa_router.callback_query(YevroremontData.filter(F.word=="yevroremont"), QashqadaryoHomeSotishHovli.remont)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YevroremontData):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.jihozlar)


@qashqa_router.callback_query(TamirlangantData.filter(F.word=="tamirlangan"), QashqadaryoHomeSotishHovli.remont)
...
@qashqa_router.callback_query(OrtachaData.filter(F.word=="ortacha"), QashqadaryoHomeSotishHovli.remont)
...
@qashqa_router.callback_query(TamirsizData.filter(F.word=="tamirsiz"), QashqadaryoHomeSotishHovli.remont)
...
# ===============================================================
@qashqa_router.callback_query(MavjudData.filter(F.word=="mavjud"), QashqadaryoHomeSotishHovli.jihozlar)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: MavjudData):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.gaz)


@qashqa_router.callback_query(JihozlarsizData.filter(F.word=="jihozlarsiz"), QashqadaryoHomeSotishHovli.jihozlar)
...
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.svet)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.gaz)
...
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Свет ✔️"
    # await callback_query.answer("Танланди")
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.suv)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Танланди")
...
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Сув ✔️"
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.kanal)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@qashqa_router.callback_query(BorData.filter(F.word=="bor"), QashqadaryoHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Канализация ✔️"
    # await callback_query.answer("Tanlandi")
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(QashqadaryoHomeSotishHovli.qoshimchaMalumot)
    else:
        await state.set_state(QashqadaryoHomeSotishHovli.qoshimchaMalumot)


@qashqa_router.callback_query(YoqData.filter(F.word=="yoq"), QashqadaryoHomeSotishHovli.kanal)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@qashqa_router.message(QashqadaryoHomeSotishHovli.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.valyuta)


# =================================================================

@qashqa_router.callback_query(USDData.filter(F.word=="usd"), QashqadaryoHomeSotishHovli.valyuta)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.narxi)


@qashqa_router.callback_query(SUMData.filter(F.word=="sum"), QashqadaryoHomeSotishHovli.valyuta)
...
@qashqa_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    QashqadaryoHomeSotishHovli.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@qashqa_router.message(QashqadaryoHomeSotishHovli.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.manzil)


@qashqa_router.message(QashqadaryoHomeSotishHovli.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.moljal)


@qashqa_router.message(QashqadaryoHomeSotishHovli.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(QashqadaryoHomeSotishHovli.telNumberOne)


@qashqa_router.message(QashqadaryoHomeSotishHovli.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(QashqadaryoHomeSotishHovli.telNumberTwo)
    else:
        await state.set_state(QashqadaryoHomeSotishHovli.telNumberTwo)


@qashqa_router.message(QashqadaryoHomeSotishHovli.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [qashqadaryoregion, data2, data3, data4, oshxona, hammom, data6, data7, data8, data9, gaz, svet, suv, kanal,
                  data10,
        ...
        await state.set_state(QashqadaryoHomeSotishHovli.check)
...
@qashqa_router.message(QashqadaryoHomeSotishHovli.check)
async def check(message: types.Message, state: FSMContext):
    ...
    if mycheck == "✅ Эълонни жойлаш":
        ...
        if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
            ...
            await bot.send_message(chat_id=group_id, text="Буйруқ Беринг", reply_markup=qashqa_group)
...
@qashqa_router.callback_query(IsGroupCall(), TasdiqlashQashqa.filter(F.word == "qashqatasdiqlash"))
...
@qashqa_router.callback_query(IsGroupCall(), BekorQilishQashqa.filter(F.word=="qashqabekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
