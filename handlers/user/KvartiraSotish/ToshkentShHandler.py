...
from states.KvartiraState.ToshShState import ToshkentshHomeSotish
...
from keyboards.inline.data import ToshketnShKvartiraData
...
toshsh_kv_router = Router()

@toshsh_kv_router.callback_query(ToshketnShKvartiraData.filter(F.word=="toshshkv"))
async def first(callback_query: CallbackQuery, state: FSMContext, callback_data: ToshketnShKvartiraData):
    # await callback_query.answer(hovlitanlandi)
    await callback_query.message.answer(rasmlar, parse_mode="HTML")

    await state.set_state(ToshkentshHomeSotish.images)


@toshsh_kv_router.message(ToshkentshHomeSotish.images, F.media_group_id, F.content_type.in_({'photo'}))
@media_group_handler
async def album_handler(messages: List[types.Message], state: FSMContext):
    ...
    await state.set_state(ToshkentshHomeSotish.umumiyMaydon)



@toshsh_kv_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    ToshkentshHomeSotish.umumiyMaydon)
async def check_umumiy(message: Message):
    await message.reply(faqatRaqamyoz)


@toshsh_kv_router.message(ToshkentshHomeSotish.umumiyMaydon)
async def umumiymaydon(message: Message, state: FSMContext):
    ...
    await state.set_state(ToshkentshHomeSotish.xonalar)

    await bot.send_message(chat_id=message.chat.id, text=xonalaryoz, parse_mode="HTML")


@toshsh_kv_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    ToshkentshHomeSotish.xonalar)
async def check_xonalar(message: Message):
    await message.reply(faqatRaqamyoz)


@toshsh_kv_router.message(ToshkentshHomeSotish.xonalar)
async def umumiymaydon(message: Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "xonalar": text
    ...
    await state.set_state(ToshkentshHomeSotish.qavat)


@toshsh_kv_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    ToshkentshHomeSotish.qavat)
async def check_qavat(message: types.Message):
    await message.reply(faqatRaqamyoz)


@toshsh_kv_router.message(ToshkentshHomeSotish.qavat)
async def umumiymaydon(message: types.Message, state: FSMContext):
    ...
    await state.set_state(ToshkentshHomeSotish.qavatlik)


@toshsh_kv_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    ToshkentshHomeSotish.qavatlik)
async def check_qavat(message: types.Message):
    await message.reply(faqatRaqamyoz)


@toshsh_kv_router.message(ToshkentshHomeSotish.qavatlik)
async def umumiymaydon(message: types.Message, state: FSMContext):
    text = message.text
    ...
    await state.set_state(ToshkentshHomeSotish.remont)


# ====================================================================
@toshsh_kv_router.callback_query(YevroremontData.filter(F.word=="yevroremont"), ToshkentshHomeSotish.remont)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YevroremontData):
    ...
    await state.set_state(ToshkentshHomeSotish.jihozlar)


@toshsh_kv_router.callback_query(TamirlangantData.filter(F.word=="tamirlangan"), ToshkentshHomeSotish.remont)
...
@toshsh_kv_router.callback_query(OrtachaData.filter(F.word=="ortacha"), ToshkentshHomeSotish.remont)
...
@toshsh_kv_router.callback_query(TamirsizData.filter(F.word=="tamirsiz"), ToshkentshHomeSotish.remont)
...
# ===============================================================
@toshsh_kv_router.callback_query(MavjudData.filter(F.word=="mavjud"), ToshkentshHomeSotish.jihozlar)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: MavjudData):
    ...
    await state.set_state(ToshkentshHomeSotish.gaz)


@toshsh_kv_router.callback_query(JihozlarsizData.filter(F.word=="jihozlarsiz"), ToshkentshHomeSotish.jihozlar)
...
@toshsh_kv_router.callback_query(BorData.filter(F.word=="bor"), ToshkentshHomeSotish.gaz)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    ...
    await state.set_state(ToshkentshHomeSotish.svet)


@toshsh_kv_router.callback_query(YoqData.filter(F.word=="yoq"), ToshkentshHomeSotish.gaz)
...
@toshsh_kv_router.callback_query(BorData.filter(F.word=="bor"), ToshkentshHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Свет ✔️"
    # await callback_query.answer("Танланди")
    ...
    await state.set_state(ToshkentshHomeSotish.suv)


@toshsh_kv_router.callback_query(YoqData.filter(F.word=="yoq"), ToshkentshHomeSotish.svet)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Танланди")
...
@toshsh_kv_router.callback_query(BorData.filter(F.word=="bor"), ToshkentshHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: BorData):
    text = " Сув ✔️"
    ...
    if callback_query.message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(ToshkentshHomeSotish.qoshimchaMalumot)
    else:
        await state.set_state(ToshkentshHomeSotish.qoshimchaMalumot)


@toshsh_kv_router.callback_query(YoqData.filter(F.word=="yoq"), ToshkentshHomeSotish.suv)
async def xonalar(callback_query: types.CallbackQuery, state: FSMContext, callback_data: YoqData):
    text = "doesnotexist"
    # await callback_query.answer("Tanlandi")
...
@toshsh_kv_router.message(ToshkentshHomeSotish.qoshimchaMalumot)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
    ...
    await state.set_state(ToshkentshHomeSotish.valyuta)


# =================================================================

@toshsh_kv_router.callback_query(USDData.filter(F.word=="usd"), ToshkentshHomeSotish.valyuta)
async def kvartira(callback_query: types.CallbackQuery, state: FSMContext, callback_data: USDData):
    ...
    await state.set_state(ToshkentshHomeSotish.narxi)


@toshsh_kv_router.callback_query(SUMData.filter(F.word=="sum"), ToshkentshHomeSotish.valyuta)
...
@toshsh_kv_router.message(lambda message: message.text and not message.text.replace('.', '').replace(',', '').isdigit(),
                    ToshkentshHomeSotish.narxi)
async def check_narxi(message: types.Message):
    await message.reply(faqatRaqamyoz)


@toshsh_kv_router.message(ToshkentshHomeSotish.narxi)
async def kvartira_narxi(message: types.Message, state: FSMContext):
    ...
    await state.set_state(ToshkentshHomeSotish.manzil)


@toshsh_kv_router.message(ToshkentshHomeSotish.manzil)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "manzil": text
    ...
    await state.set_state(ToshkentshHomeSotish.moljal)


@toshsh_kv_router.message(ToshkentshHomeSotish.moljal)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "moljal": text
    ...
    await state.set_state(ToshkentshHomeSotish.telNumberOne)


@toshsh_kv_router.message(ToshkentshHomeSotish.telNumberOne)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    ...
    if message.text == "⏭️ Кейингиси":
        ...
        await state.set_state(ToshkentshHomeSotish.telNumberTwo)
    else:
        await state.set_state(ToshkentshHomeSotish.telNumberTwo)


@toshsh_kv_router.message(ToshkentshHomeSotish.telNumberTwo)
async def umumiyMaydon(message: types.Message, state: FSMContext):
    text = message.text
    await state.update_data({
        "telNumberTwo": text
    ...
    if data['qoshimchaMalumot'] == "⏭️ Кейингиси" and data['telNumberTwo'] == "⏭️ Кейингиси":
        ...
        result = [toshkentshregion, data2, data3, data4, data5, data6, data7, data8, data9, gaz, svet, suv, data10,
...
@toshsh_kv_router.message(ToshkentshHomeSotish.check)
...
@toshsh_kv_router.callback_query(IsGroupCall(), TasdiqlashToshsh.filter(F.word == "toshshtasdiqlash"))
...
@toshsh_kv_router.callback_query(IsGroupCall(), BekorQilishToshsh.filter(F.word=="toshshbekor"))
async def dokumentlar(call: types.CallbackQuery, state: FSMContext):
